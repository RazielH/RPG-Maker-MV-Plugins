...
(function ($$) {
  ...
  $$.DayNight = function (args) {
    ...
    function getHourValue(hour) {
      let h = Number(hour);
      if (h < 0) h = 0;
      if (h >= daynighthoursinday) h = (daynighthoursinday - 1);
      return h;
    }

    function setDayNightColor(hour, hourColor) {
      let isValidColor = isValidColorRegex.test(hourColor);
      if (isValidColor) {
        daynightcolors[getHourValue(hour)].color = hourColor;
      }
      $gameVariables.SetDaynightColorArray(daynightcolors);
    }
    ...
    if (args[0] === 'color') {
      ...
      setDayNightColor(hour, hourColor);
    }

    if (args[0] === 'colorRange') {
      let hourStart = getHourValue(args[1]);
      let hourEnd = getHourValue(args[2]);
      for (let hour = hourStart; ;) {
        let color = args[3];
        setDayNightColor(hour, color);
        if (hour === hourEnd) break;
        hour = ++hour % daynighthoursinday;
      }
    }
  };
})(Community.Lighting);
